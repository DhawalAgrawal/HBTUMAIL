...
<?php $c = mysql_query("SELECT * FROM  `sentmail` WHERE  `senderrid` LIKE  '".$_SESSION['rid']."' AND  `mailid` LIKE  '".$_GET['mailid']."'") ; $d = mysql_fetch_array($c); ?>
<?php if(!$d['id']){ header( 'location:sent.php' );  exit(); } ?>
...
<div id="me">
        <div class="postHeadline"> Messages <a href="sent.php">back</a></div>
        <div class="hDevider"></div>
       <?php $c = mysql_query("SELECT * FROM  `sentmail` WHERE  `senderrid` LIKE  '".$_SESSION['rid']."' AND  `mailid` LIKE  '".$_GET['mailid']."'") ; $d = mysql_fetch_array($c); ?>
         
        <span class="backendHeadline"> <span> Subject : <?php if($d['sub']){ echo '<font color="#000">'. $d['sub'].'</font>'.','; }$date = date_create($d['date']);?> </span>
        <span style="font-weight:normal; font-size:12px; color: #b7b7b7;"> <?=date_format($date, 'j M y')?> </span>
        </span>
        
        <div class="mailBox">
        <span>To : </span> 
        <?php
		$each = explode('*',$d['receiverrid']);
		$total = count($each);
		if($total===1){ $a = mysql_query("SELECT * FROM  `mailmembers` WHERE  `rid` LIKE  '".$d['receiverrid']."' AND  `yn` =1") ; $b = mysql_fetch_array($a);
		echo  $b['name'];
		}else{
		$info = new SplFileInfo($each[0]);$a = mysql_query("SELECT * FROM  `mailmembers` WHERE  `rid` LIKE  '".$info."' AND  `yn` =1") ; $b = mysql_fetch_array($a);$to=$total-1;
        echo $b['name']. ' and '. $to .' other.' ;}
		?>
               <div class="boxDevider" ></div> <?=$d['message']?> 
			   <?php if($d['files']!=''){ ?>
                 <div class="boxDevider" ></div>
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
        <td width="100%" align="left" valign="middle"><span>Downloads</span> </td>
        </tr>
        <tr>
        <td width="100%" align="left" valign="middle">
		<?php $c = mysql_query("SELECT * FROM  `sentmail` WHERE  `senderrid` LIKE  '".$_SESSION['rid']."' AND  `mailid` LIKE  '".$_GET['mailid']."'") ; $d = mysql_fetch_array($c); ?>
		<?php
		$allfiles = explode('*',$d['files']);
		$totalfiles = count($allfiles);
		for ($z = 0; $z <= $totalfiles-1; $z++) {
		$info = new SplFileInfo($allfiles[$z]);
        $ext  = $info->getExtension();
		echo '<a href="uploadFiles/'.$allfiles[$z].'" />' ; ?>
        
        <?php
		if( $ext=='jpg' or $ext=='JPEG' or $ext=='jpeg' or $ext=='png' or $ext=='gif' ){
		echo '<img src="images/imgfile.png" />' ; } else if( $ext=='doc' or $ext=='docx' ){
		echo '<img src="images/docfile.png" />' ; }else if( $ext=='csv' or $ext=='xlsx' ){
		echo '<img src="images/exelfile.png" />' ; }else if( $ext=='zip' or $ext=='rar' ){ 
		echo '<img src="images/rarfile.png" />' ; }else if( $ext=='pdf'){
		echo '<img src="images/pdffile.png" />' ; }else{ echo '<img src="images/afile.png" />' ; }
		echo '</a>' ; }
		?>
